...
## Verification & literature comparison
...
`reproduce.py` over the same 27-scenario suite (both trophic modes, both carbon sources,
...
**Comparison against published studies — is it plausible?** None of it is prospective
validation: every source was already in print when its scenario was built. The 12 point
comparisons are therefore classified by **what each one depends on**, and never pooled:

| Class | n | Agreement |
|---|---:|---|
| retrospective untuned — production cost | 4 scenarios, 2 publications | −14% to +1.5% |
| retrospective untuned — cradle-to-gate GWP | 3 scenarios, 3 publications | −11% to +4% |
| component-informed — production cost | 1 | −4%, with 48.28 % of the compared numerator built from the source's own published OPEX |
| calibrated benchmarks | 4 | −5% to +6% |

Plus 1 range plausibility check, which the engine **fails** and which is reported anyway, and
3 records excluded because their figures could not be traced to an eligible primary source.
...
what they do *not* establish, **[docs/VALIDATION.md](docs/VALIDATION.md)** for the comparison
tables and the evidence classes, [docs/STUDY_SELECTION.md](docs/STUDY_SELECTION.md) for how the studies were chosen (and
...
> for microalgae and aquatic protist biomass* (Version 1.5.0) [Computer software]. Zenodo.
...
version you ran, cite its own DOI instead. v1.5.0 is
[`10.5281/zenodo.22143048`](https://doi.org/10.5281/zenodo.22143048), v1.4.0 is
