...
runs of this script on the same commit produce byte-identical output.
...
STAGES = ("studies", "endpoints", "verification", "reproductions", "harmonization",
          "gwp", "carbon", "sobol", "uncertainty", "figures", "summary")
...
def stage_verification(run: Run) -> None:
    """Internal verification: do the conserved quantities close?
    ...
    """
    print("[verification] mass-balance closure and model invariants")
    blocks, ok = [], True
    checked = 0
    for rec in sorted((r for r in run.dataset if r.is_executable), key=lambda r: r.study_id):
        rep = verify(reconstructions.build(rec.reconstruction_builder, run.lib))
        ok = ok and rep.all_pass
        checked += 1
        blocks.append(format_report(rec.study_id, rep))
    for arch in archetypes.ARCHETYPES:
        if arch.kind != "library_default":
            continue
        rep = verify(arch.build(run.lib))
        ok = ok and rep.all_pass
        checked += 1
        blocks.append(format_report(f"archetype:{arch.key}", rep))

    header = report.header(
        "INTERNAL VERIFICATION",
        "carbon, nitrogen and phosphorus closure, product-mass and scale-invariance "
        "invariants - self-consistency, NOT agreement with an external reference",
    )
    header += [
    ...
    run.notes.append(
        f"Mass-balance verification over {checked} scenarios: {'PASS' if ok else 'FAIL'}."
    )
    if not ok:
...
def stage_reproductions(run: Run) -> None:
    ...
    _write(RESULTS / "validation.txt", report.reproductions(rows, blocked))
    ...
    run.notes.append(
        f"{n_point} point reproduction(s) and {len(blocked)} blocked Tier-B studies."
    )
...
def stage_sobol(run: Run) -> None:
    ...
    passed = all(o.max_abs_error <= SOBOL_BENCHMARK_TOLERANCE
                 and not o.result.violations() for o in outcomes)
...
def stage_uncertainty(run: Run) -> None:
    ...
    run.status["task_6_uncertainty_decomposition"] = "pass"
...
def stage_summary(run: Run) -> None:
    ...
    task_stages = ("studies", "endpoints", "verification", "reproductions",
                   "harmonization", "gwp", "carbon", "sobol", "uncertainty")
